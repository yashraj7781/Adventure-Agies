...
async function autoCompleteSave(text, obj, userId, languageId, premium_status, context, currentLine) {
		...
		if (text.length) {
			...
			if (text.length>10){
				...
				if (premium_status){
					...
					let url_request = ''
					try{
						const response = await (0, node_fetch_1.default)('https://www.useblackbox.io/suggestv4', {
							method: 'POST',
							body: JSON.stringify({
								inputCode: text,
